...
Agent-facing project notes for this repository.
...
Generated files include route pages (`index.html`, `*/index.html`) plus `robots.txt` and `sitemap.xml`.
...
- Preserve `CNAME`.
- Keep external links opening in new tab (`target="_blank"`, `rel="noopener noreferrer"`). This is enforced by build logic.
